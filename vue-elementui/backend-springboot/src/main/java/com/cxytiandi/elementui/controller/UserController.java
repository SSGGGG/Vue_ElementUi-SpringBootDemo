...
public class UserController {
	...
	public ResponseData login(String username, String password) {
		...
		if (username.equals("root") && password.equals("root")) {
			...
			MenuVo roleManage = new MenuVo();
			roleManage.setIndex("/user/role");
			roleManage.setTitle("角色管理");
			
			MenuVo m2 = new MenuVo();
			m2.setIcon("el-icon-menu");
			m2.setIndex("2");
			m2.setTitle("用户管理");
			m2.setSubs(Arrays.asList(m3, m4, roleManage));
			meuns.add(m2);
			...
			result.put("meuns", meuns);
			// 这里的routers记得修改
			result.put("routers", "/home_/users_/user/info_/user/role");
			return ResponseData.ok(result);
		}
